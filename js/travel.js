...
(function () {
  ...
  function getItems() {
    return HUTBData.list(SECTION).filter((it) => {
      ...
      if (searchText) {
        const blob = [
          it.title, it.excerpt, it.location, it.transport,
    ...
    grid.innerHTML = items.map((it) => {
      ...
      return `
        <article class="card fade-up" onclick="location.href='${articleUrl}'" style="cursor:pointer;">
          <div class="card-cover" style="background: linear-gradient(135deg, #52a6b8 0%, #2989b8 100%); color:#fff;">
            ${it.cover ? `<img src="${HUTB.escape(it.cover)}" alt="">` : `<span style="font-size:4rem;">🏖️</span>`}
            ${it.pinned ? `<span class="card-cover-pin">📌 推荐</span>` : ''}
            <span class="card-cover-tag">${HUTB.escape(it.category || '景点')}</span>
          </div>
          <div class="card-body">
            ...
            <div class="card-meta">
              ${it.duration ? `<span class="card-meta-row">⏱️ ${HUTB.escape(it.duration)}</span>` : ''}
              ${it.bestSeason ? `<span class="card-meta-row">🌤️ ${HUTB.escape(it.bestSeason)}</span>` : ''}
              <span style="margin-left:auto; color:#1e6091; font-weight: 600;">${HUTB.escape(it.cost || '免费')}</span>
            </div>
            ${it.transport ? `<div style="margin-top: var(--sp-2); font-size: 0.825rem; color:var(--c-text-muted);">🚆 ${HUTB.escape(it.transport)}</div>` : ''}
          </div>
        </article>
      `;
    }).join('');
